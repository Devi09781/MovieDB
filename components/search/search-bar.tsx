...
export function SearchBar() {
  ...
  const trending = dataRepository.getTrendingSearches();
  ...
  const highlight = (text: string, q: string) => {
    ...
    return (<>{text.slice(0, idx)}<mark className="bg-brand-500/30 text-brand-300 rounded px-0.5">{text.slice(idx, idx + q.length)}</mark>{text.slice(idx + q.length)}</>);
  };

  const navigate = (path: string) => { setOpen(false); setQuery(""); startTransition(() => router.push(path)); };
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    else if (e.key === "Enter" && activeIndex >= 0 && results[activeIndex]) { e.preventDefault(); navigate(`/movie/${results[activeIndex].id}`); }
  };
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-xl border border-ink-700 glass shadow-card">
          {query.length > 1 && results.length > 0 && (
            <ul>{results.slice(0, 6).map((movie, i) => (<li key={movie.id} role="option" aria-selected={i === activeIndex}><Link href={`/movie/${movie.id}`} onMouseEnter={() => setActiveIndex(i)} onClick={() => { setOpen(false); setQuery(""); }} className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${i === activeIndex ? "bg-ink-700" : "hover:bg-ink-700/50"}`}><span className="h-10 w-8 flex-shrink-0 rounded bg-ink-600" /><div className="min-w-0"><p className="truncate font-medium text-white">{highlight(movie.title, query)}</p><p className="truncate text-xs text-zinc-400">{movie.year} · {movie.genres.join(", ")}</p></div></Link></li>))}</ul>
          )}
          {query.length > 1 && results.length === 0 && <p className="px-4 py-6 text-center text-sm text-zinc-400">No results for &quot;{query}&quot;</p>}
          {query.length <= 1 && (
            <div className="px-4 py-3"><p className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-zinc-400"><TrendingUp className="h-3 w-3" />Trending searches</p><div className="flex flex-wrap gap-2">{trending.map((t) => <button key={t} onClick={() => { setQuery(t); inputRef.current?.focus(); }} className="rounded-full bg-ink-700 px-3 py-1 text-xs text-zinc-300 hover:bg-brand-500 hover:text-ink-950 transition-colors">{t}</button>)}</div></div>
          )}
        </div>
      )}
    </div>
