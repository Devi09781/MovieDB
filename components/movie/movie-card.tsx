    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: Math.min(index * 0.04, 0.3) }} whileHover={{ y: -8, scale: 1.03 }} className="group relative w-[160px] flex-shrink-0 sm:w-[200px]">
      <Link href={`/movie/${movie.id}`} className="block">
        <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-ink-800 shadow-card">
          <Image src={movie.posterUrl} alt={movie.title} fill sizes="(max-width: 640px) 160px, 200px" className="object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
          <div className="absolute left-2 top-2"><span className="inline-flex items-center gap-1 rounded-md bg-black/70 px-1.5 py-0.5 text-xs font-semibold text-brand-400 backdrop-blur"><Star className="h-3 w-3 fill-current" />{movie.rating.toFixed(1)}</span></div>
      ...
      <div className="mt-2"><Link href={`/movie/${movie.id}`}><h3 className="line-clamp-1 text-sm font-semibold text-white hover:text-brand-400 transition-colors">{movie.title}</h3></Link><p className="text-xs text-zinc-400">{movie.year}</p></div>
