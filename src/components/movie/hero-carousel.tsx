        ...
        <div className="px-4 sm:px-8 lg:px-16">
          <motion.div key={`info-${movie.id}`} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="max-w-2xl">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-zinc-300">
              <span className="font-semibold text-brand-400">★ {Number(movie.rating).toFixed(1)}</span><span>{movie.year}</span><span>{movie.runtime}m</span>
            ...
            <div className="mt-6 flex gap-3">
              <Link to={`/movie/${movie.id}`}><span className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-6 py-3 font-semibold text-ink-950 transition-all hover:bg-brand-400 hover:shadow-glow"><Play className="h-5 w-5 fill-current" />Watch Trailer</span></Link>
              <Link to={`/movie/${movie.id}`}><span className="inline-flex items-center gap-2 rounded-lg glass px-6 py-3 font-semibold text-white transition-all hover:bg-ink-700"><Info className="h-5 w-5" />More Info</span></Link>
            </div>
          </motion.div>
        </div>
      </div>
