...
export function Rail({ title, children, seeAllTo }: { title: string; children: ReactNode; seeAllTo?: string }) {
  ...
  return (
    <section className="group/rail relative px-4 py-6 sm:px-8">
      <div className="mb-3 flex items-center justify-between">
        ...
        {seeAllTo && <Link to={seeAllTo} className="text-sm font-medium text-brand-400 hover:text-brand-300 transition-colors">See all</Link>}
      </div>
