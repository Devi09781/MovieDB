      ...
      <div className="relative z-10 flex h-full items-end pb-8">
        <div className="mx-auto flex w-full max-w-6xl gap-6 px-4 sm:px-8">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="hidden w-40 flex-shrink-0 sm:block">
            <div className="relative aspect-[2/3] overflow-hidden rounded-xl shadow-card"><img src={movie.poster_url} alt={movie.title} className="h-full w-full object-cover" /></div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="flex-1">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-zinc-300">
              <span className="flex items-center gap-1 font-semibold text-brand-400"><Star className="h-4 w-4 fill-current" />{Number(movie.rating).toFixed(1)}</span>
            ...
            <div className="mt-6 flex gap-3">
              {movie.trailer_url && <a href={movie.trailer_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-6 py-3 font-semibold text-ink-950 transition-all hover:bg-brand-400 hover:shadow-glow"><Play className="h-5 w-5 fill-current" />Play Trailer</a>}
