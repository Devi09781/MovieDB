...
import { useReviews, useAddReview, useUpdateReview, useDeleteReview, useVoteReview } from "@/hooks/use-data";
...
export function ReviewsSection({ movieId }: { movieId: string }) {
  ...
  const onSubmit = (data: ReviewForm) => {
    if (editingId) {
      updateReview.mutate({ id: editingId, updates: { rating: data.rating, title: data.title, content: data.content } });
      setEditingId(null);
    } else {
      addReview.mutate({ movie_id: movieId, user_id: USER_ID, user_name: USER_NAME, rating: data.rating, title: data.title, content: data.content });
    }
    form.reset({ rating: 8, title: "", content: "" });
    setShowForm(false);
  };

  const startEdit = (review: Review) => {
    setEditingId(review.id);
    form.reset({ rating: review.rating, title: review.title, content: review.content });
    setShowForm(true);
  };
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} onSubmit={form.handleSubmit(onSubmit)} className="mb-6 overflow-hidden rounded-xl border border-ink-700 bg-ink-800 p-5">
            ...
            <button type="submit" disabled={addReview.isPending || updateReview.isPending} className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2 text-sm font-semibold text-ink-950 transition-all hover:bg-brand-400 disabled:opacity-60">{(addReview.isPending || updateReview.isPending) && <Loader2 className="h-4 w-4 animate-spin" />}{editingId ? "Update Review" : "Post Review"}</button>
          </motion.form>
        )}
      </AnimatePresence>
          ...
          <AnimatePresence>{paginated.map((review) => (
            <motion.article key={review.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="rounded-xl border border-ink-700 bg-ink-800 p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500/20 text-brand-400 font-semibold">{review.user_name.charAt(0)}</div><div><p className="font-medium text-white">{review.user_name}</p><p className="text-xs text-zinc-400">{new Date(review.created_at).toLocaleDateString()}{review.edited && " · edited"}</p></div></div>
              ...
              <div className="mt-4 flex items-center gap-4">
                <button onClick={() => voteReview.mutate(review.id)} className="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-brand-400 transition-colors"><ThumbsUp className="h-3.5 w-3.5" />Helpful ({review.helpful_count})</button>
                {review.user_id === USER_ID && (<><button onClick={() => startEdit(review)} className="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-brand-400 transition-colors"><Edit3 className="h-3.5 w-3.5" />Edit</button><button onClick={() => deleteReview.mutate(review.id)} className="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-red-400 transition-colors"><Trash2 className="h-3.5 w-3.5" />Delete</button></>)}
              </div>
            </motion.article>
          ))}</AnimatePresence>
...
export function ReviewsSection({
  ...
  const onSubmit = (data: ReviewForm) => {
    if (editingId) {
      ...
      // user_id and user_name are not accepted by useAddReview.
      addReview.mutate({
        movie_id: movieId,
        rating: data.rating,
        title: data.title,
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-semibold text-ink-950 transition-all hover:bg-brand-400"
        >
          {showForm ? (
            <X className="h-4 w-4" />
          ) : (
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form
            ...
            className="mb-6 overflow-hidden rounded-xl border border-ink-700 bg-ink-800 p-5"
          >
            <div className="mb-3">
            ...
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full rounded-lg border border-ink-600 bg-ink-900 px-3 py-2 text-sm text-white focus:border-brand-500 focus:outline-none"
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2 text-sm font-semibold text-ink-950 transition-all hover:bg-brand-400 disabled:opacity-60"
            >
              {(addReview.isPending ||
                updateReview.isPending) && (
      ...
      {isLoading ? (
        ...
        <p className="rounded-xl border border-ink-700 bg-ink-800 p-8 text-center text-zinc-400">
        ...
        <div className="space-y-4">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500/20 font-semibold text-brand-400">
                    ...
                    <span className="flex items-center gap-1 rounded-md bg-ink-700 px-2 py-1 text-sm font-semibold text-brand-400">
                  ...
                  <div className="mt-4 flex items-center gap-4">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs text-zinc-400 transition-colors hover:text-brand-400"
                    >
                    ...
                    {review.user_id ===
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-xs text-zinc-400 transition-colors hover:text-red-400"
                        >
          ...
          {hasMore && (
            <button
              ...
              className="w-full rounded-xl border border-ink-700 bg-ink-800 py-3 text-sm font-medium text-zinc-300 transition-colors hover:bg-ink-700 hover:text-white"
            >
              Load more reviews
            </button>
          )}
